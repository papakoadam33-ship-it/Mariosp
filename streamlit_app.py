...
# --- ΡΥΘΜΙΣΕΙΣ API ---
API_KEY = "a963742bcd5642afbe8c842d057f25ad" # <--- ΒΑΛΕ ΤΟ ΚΛΕΙΔΙ ΠΟΥ ΔΟΥΛΕΥΕΙ
...
def get_form_string(team_name, matches_list):
    if not matches_list: return "N/A", ["Δεν βρέθηκαν δεδομένα"]
    form, details = [], []
    for m in matches_list:
        ...
        if h_g == a_g: icon, txt = "🟡", f"Ισοπαλία {h_g}-{a_g} με {opp}"
        elif (is_home and h_g > a_g) or (not is_home and a_g > h_g): icon, txt = "🟢", f"Νίκη {h_g}-{a_g} με {opp}"
        else: icon, txt = "🔴", f"Ήττα {h_g}-{a_g} με {opp}"
        form.append(icon)
        details.append(txt)
        if len(form) == 5: break
...
# --- ΒΑΘΜΟΛΟΓΙΑ ---
st.sidebar.markdown(f"### 🏆 Βαθμολογία")
st_data = fetch_data(f"https://api.football-data.org/v4/competitions/{sel_league_code}/standings")
if st_data:
    table = [{"#": e['position'], "Ομάδα": e['team']['shortName'], "Β": e['points']} 
             for e in st_data.get('standings', [{}])[0].get('table', [])]
    st.sidebar.table(pd.DataFrame(table).set_index('#'))

# --- ΑΓΩΝΕΣ ---
data = fetch_data(f"https://api.football-data.org/v4/competitions/{sel_league_code}/matches")
all_matches = data.get('matches', [])
upcoming = [m for m in all_matches if m['status'] in ['SCHEDULED', 'TIMED', 'LIVE']]

if not upcoming:
    st.warning("⚠️ Δεν υπάρχουν live ή μελλοντικοί αγώνες λόγω διακοπής. Εμφάνιση τελευταίων αποτελεσμάτων:")
    upcoming = [m for m in all_matches if m['status'] == 'FINISHED'][-10:]
else:
...
for m in upcoming:
    ...
    with st.expander(f"📅 {status} | {h_t} vs {a_t}"):
        ...
        st.divider()
        st.subheader("📊 Φόρμα (Τελευταία 5)")
        ...
        f_h, d_h = get_form_string(h_t, h_d.get('matches', []))
        f_a, d_a = get_form_string(a_t, a_d.get('matches', []))
        with col_h:
            st.write(f"🏠 **{h_t}**"); st.markdown(f"### {f_h}")
            for d in d_h: st.caption(d)
        with col_a:
            st.write(f"🚀 **{a_t}**"); st.markdown(f"### {f_a}")
