...
st.markdown("""
    ...
    /* 3. FIX ΓΙΑ ΤΟ ΑΣΠΡΙΣΜΑ ΤΩΝ EXPANDERS */
    /* Επιβάλλουμε σκούρο χρώμα ακόμα και όταν είναι ανοιχτά ή πατημένα */
    ...
    .streamlit-expanderHeader:hover, .streamlit-expanderHeader:focus, .streamlit-expanderHeader[aria-expanded="true"] {
        background-color: rgba(255, 255, 255, 0.2) !important; /* Λίγο πιο ανοιχτό αλλά ΟΧΙ λευκό */
...
# ΔΙΟΡΘΩΜΕΝΗ ΣΥΝΑΡΤΗΣΗ ΓΙΑ ΤΟ ΠΡΑΣΙΝΟ ΧΡΩΜΑ ΣΤΑ 70%+
...
# --- SIDEBAR ---
st.sidebar.title("⚙️ Predictor Menu")
...
# Φιλτράρουμε μόνο τα μελλοντικά ή live ματς
display_m = [m for m in all_data.get('matches', []) if m['status'] in ['SCHEDULED', 'TIMED', 'IN_PLAY', 'PAUSED']][:30]

for i, m in enumerate(display_m):
    # Διαχωρισμός ανά αγωνιστική
    if i > 0 and i % matches_per_round == 0:
        st.markdown('<div class="matchday-divider"></div><div class="matchday-label">Next Matchday Block</div>', unsafe_allow_html=True)
        ...
        for idx in range(6):
            cols[idx].markdown(f"""
                <div style="text-align:center; background:rgba(0,0,0,0.6); padding:12px; border-radius:12px; border: 1px solid rgba(255,255,255,0.1);">
                    <div style="color:#bbb; font-size:13px; margin-bottom:5px;">{lbls[idx]}</div>
